...
implementation {
  ...
  uint16_t Tpsn0SeqNr;  // 2 Bytes. It contains the sequence number of 
                         // of the last received TPSN request.
 ...
 event message_t * Receive.receive(message_t *msg, void *payload, uint8_t len){
    ...
    if ((len == sizeof(Tpsn1Msg_t))&&(call PacketTimeStampMilli.isValid(msg))) {
      //Check passed! Cast the payload pointer to TPNS request form L1 node
    ...
    //First. Cast our packet (Tpsn0_ReportMsg_t ) into a "report TPSN " type
    //Refer to the packet by using the "Tpsn0rMsg" pointer.
    Tpsn0rMsg = (Tpsn0_ReportMsg_t * )(call Packet.getPayload(&pktBuff, sizeof(Tpsn0_ReportMsg_t)));
 
    //Check that the casting was ok. 
    if (Tpsn0rMsg == NULL){
      // Casting failed for any reason, the pointer Tpsn0rMsg is NULL. 
      // Do nothing and leave this function.
      return msg;
    }  
 
    //Otherwise, the pointer Tpsn0rMsg is valid! Prepare packet!
 
    Tpsn0rMsg->nodeId  = TOS_NODE_ID;     // This ID of this node (installed value) 
    Tpsn0rMsg->Tpsn0SeqNr  = Tpsn0SeqNr;     // Set last TPSN request's sequence number

    // Set the radio and application time stamps for the last TPSN request
    ...
    // is L1_ADD_Node, which is defined as 0x000A and it means that 
    // L1 node around will accept the packet.
  ...
  // When we receive a TPSN request, we store the information about 
