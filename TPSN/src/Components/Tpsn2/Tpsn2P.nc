...
implementation {
 ...
 message_t pktBuff;            // many Bytes, packet buffer. This is not a pointer,
  ...
  event void AMSend.sendDone(message_t * msg, error_t err) {
    ...
    // application (Tpsn2C). Check that the pointer msg points to pktBuff   
    if(&pktBuff == msg) {
...
// this event receive packet form L1 TPSN report message and adjust its time using TPSN protocol.   
